...
INTERPOLATION_STEPS = 10 # TODO : default is 10 based on the paper
...
test = create_sequential_model_without_top(model)
...
class GradModel(nn.Module):
    def __init__(self, model_name='resnet50', feature_layer=8):
        ...
        if model_name == 'pairnet':
            model = CNN2DsmallFeatureDiff(inputsize=224, channels=1,
                                          initial_channel=16,
                                          n_of_blocks=4, num_conv=1, pooling=nn.AvgPool2d)

            self.features = model.encoder
            self.post_features = None
            self.avgpool = None
            self.classifier = model.linear
...
def heatmap_of_layer(device, image_path, interpolation_on_activations_steps_arr,
    ...
    grads = []
    for act in igacts: # TODO Activation stage layer gradient wrt prediction
        act.requires_grad = True
...
def by_class_map(model, image, label, operation='ours', use_mask=False):
    ...
    heatmap = grad2heatmaps(model, image.unsqueeze(0).to(device), gradients, activations=None, operation=operation,
                            ...
                            weight_ratio=weight_ratio) # regular heatmap?
